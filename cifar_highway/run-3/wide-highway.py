...
for epoch in xrange(1, epochs + 1):
    ...
    if epoch in prune_at:
        ...
        np.save('max_values1'+str(epoch), max_values1)
        np.save('max_values2'+str(epoch), max_values2)
        np.save('max_values3'+str(epoch), max_values3)
    ...
    while cursor + batch_size <= len(train_x):  # So that masks are created only once -ignore last batch of smaller size
        ...
        loss.backward()
        nn.utils.clip_grad_norm(network.parameters(), 1.5)
        optimizer.step()
        cursor += batch_size
