...
def conv_highway(x, fan_in, fan_out, stride, filter_size, not_pool=False):
    ...
    if fan_in != fan_out:
        ...
        res += C * x_new
        return res, tf.reduce_max(T, axis=[1, 2])
    return (res + (C * x)), tf.reduce_max(T, axis=[1, 2])
...
with graph.as_default():
    ...
    loss = tf.reduce_mean(cross_entropy) + tc_multiplier * tf.reduce_max(transform_sum)
...
epochs = 170  # 10 * int(round(40000/batch_size)+1)
losses = []
...
with tf.Session(graph=graph) as session:
    ...
    while i <= epochs:
        ...
        if i < 31:
            ...
            feed_dict = {x: batch_xs, y: batch_ys, lr: learn_rate, tc_multiplier: 0.09}
        elif i < 100:
            feed_dict = {x: batch_xs, y: batch_ys, lr: learn_rate, tc_multiplier: 0.11}
        elif i < 120:
            feed_dict = {x: batch_xs, y: batch_ys, lr: learn_rate, tc_multiplier: 0.13}
        elif i < 140:
            feed_dict = {x: batch_xs, y: batch_ys, lr: learn_rate, tc_multiplier: 0.15}
        else:
            feed_dict = {x: batch_xs, y: batch_ys, lr: learn_rate, tc_multiplier: 0.17}
